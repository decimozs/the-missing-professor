  ...
  return (
    <div className="bg-gray-900 border border-green-400 rounded-lg p-4">
      <div className="flex items-center space-x-2 mb-4">
        ...
        <span className="text-green-300 text-sm font-semibold">DECRYPTION OUTPUT</span>
      </div>

      <div className="space-y-4">
        {/* Original message */}
        <div>
          <label className="text-green-400 text-xs block mb-2">ENCRYPTED (SHIFT: {shiftValue}):</label>
        ...
        <div>
          <label className="text-green-400 text-xs block mb-2">DECRYPTED OUTPUT:</label>
          <div className="bg-black border border-gray-600 rounded p-3 font-mono text-xs min-h-[100px]">
            {decodedMessage ? (
              <div className={`break-all ${messageDecoded ? 'text-green-400' : 'text-yellow-400'}`}>
                {showTyping ? (
                  <TypingAnimation 
                    text={decodedMessage}
                    speed={30}
        ...
        {decodedMessage && (
          <div>
            <label className="text-green-400 text-xs block mb-2">FREQUENCY ANALYSIS:</label>
            <div className="bg-black border border-gray-600 rounded p-3 text-xs">
                  ...
                  {topLetters.map(([letter, count]) => {
                    ...
                    return (
                      <div key={letter} className="text-center">
                        <div className={`text-lg font-bold ${count > 0 ? 'text-green-400' : 'text-gray-500'}`}>
                          {letter}
                        </div>
              ...
              <div className="grid grid-cols-13 gap-1 text-center">
                {Array.from("ABCDEFGHIJKLMNOPQRSTUVWXYZ").map((letter) => {
                  ...
                  const isCommon = ['E', 'T', 'A', 'O', 'I', 'N'].includes(letter);
                  return (
                    <div key={letter} className="text-green-300">
                      <div className={`text-xs ${isCommon ? 'text-yellow-400 font-bold' : ''}`}>
                        {letter}
                      </div>
                      <div className={`text-xs ${count > 0 ? 'text-green-400' : 'text-gray-600'}`}>
                        {count}
                      </div>
                    </div>
                  );
              ...
              <div className="mt-2 text-yellow-400 text-xs">
                💡 In English, 'E' is the most common letter (~12%), followed by 'T', 'A', 'O', 'I', 'N'
              </div>
            </div>
          </div>
        )}
        ...
        {messageDecoded && (
          <div className="space-y-3">
            <div className="bg-green-900 border border-green-400 rounded p-3 text-green-300 text-sm">
              ✅ Message successfully decrypted! The shift value was {shiftValue}.
              <br />
              Flag discovered: <span className="font-mono text-green-400">LOCATION_AI_TERMINAL</span>
            </div>

            {/* File download notification */}
            {aiLabNotesClue && (
              <div className="bg-blue-900 border border-blue-400 rounded p-3 text-blue-300 text-sm">
                📁 <span className="text-blue-400 font-bold">File Downloaded:</span> AI_LAB_NOTES.txt
                <br />
